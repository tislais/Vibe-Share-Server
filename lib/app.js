...
app.get('/api/mixtape/:id', async (req, res) => {
  try {
    const data = await client.query(`
      SELECT playlist_id, title, theme, note, recipient, user_id
      FROM      playlists
      WHERE   user_id = $1;
    `, [req.userId]);
...
app.get('/api/mixtape/:id/favorites', async (req, res) => {
  ...
  try {
    const data = await client.query(`
      SELECT  id, playlist_id, title, theme, note, recipient, user_id;
              u.name as "userName"
      FROM    favorites f
      JOIN    users u
      ON      f.user_id = u.id
...
app.delete('/api/favorites/:id', async (req, res) => {
  try {
    const data = await client.query(`
      ...
      WHERE        id = $1
      AND          user_id = $2
      RETURNING    (favorites_playlist_id, favorites_user_id);   
    `, [req.params.id, req.userId]);
